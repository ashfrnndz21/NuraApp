...
- Web (ADR 0001): `make web` (dev server on http://127.0.0.1:5173/app/, proxying `/api` to `make dev`), `make build-web` (writes `web/dist`, which `make dev` serves at http://127.0.0.1:8000/app/), `make web-test` (Vitest), `make web-e2e` (Playwright against the served build; it starts `make dev` itself with the clock frozen by `NURA_FROZEN_CLOCK`, a dev-run-only setting that `POST /dev/clock` moves). `make plain-words` covers `web/src/strings/**` too. `make web-mock` (the dev server with E01's onboarding routes answered by `web/src/api/mock/` until that backend merges).
...
- `web/` — the web client: `src/ui` (tokens, the two densities), `src/strings` (en, ms, zh, tagged `@patient`), `src/api` (and `src/api/mock`, dev only), `src/store`, `src/screens`, `src/today`, `src/onboarding`, `src/feed` (the vertical pager's store, cards and playback), `src/speech`, `src/sw`; `tests/unit`, `tests/e2e`.
