.PHONY: dev migrate reset-db checkpoint test lint plain-words ios-test
...
dev: migrate ; cd backend && uvicorn app.main:app --reload --no-access-log 2>&1 | tee .dev.log
...
checkpoint: ; cd backend && python -m scripts.checkpoint $(N)
