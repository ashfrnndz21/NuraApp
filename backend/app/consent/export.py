...
Whether a recording sits behind a spoken agreement is carried as a yes or no, never as the
recording's id: that id opens health data. The words come from the row, never re-derived
from the catalogue.
...
made for — "you" when that is the patient, his name when it is someone he let in. The
frame of the page is English today; the words he agreed to are printed in the language he
read them in, and the parts a person may see are named in that language too. A page whose
frame is in his language is the next step. A PDF renderer is a later adapter with the
same one method; nothing above it changes when it arrives. Whatever renders it receives
the whole document, names included.
...
`ConsentRecord.region` says which region the bytes belong to; a channel sending them
calls `guard_region` — the field is the label, not the guard.
"""
...
from app.consent.texts import LANGUAGES, current_version, what_lines
...
def basis_lines(
    ...
    *,
    ...
    """The lines that say why `giver` could agree for `patient`, one idea each. Empty when
    he agreed himself.

    Names arrive already in the reader's voice ("you"/"your", or a name and "Pa's").
    """
    match basis:
        ...
        case ConsentBasis.LPA:
            return [
                f"{giver} holds a paper about {patients} health.",
                f"That paper lets {giver} decide for {patient}.",
            ]
        ...
        case ConsentBasis.VERBAL_RECORDED:
            lines = [f"{patient} said yes out loud."]
            if witness is not None:
                lines.append(f"{witness} was there and heard {patient} say it.")
            lines.append(f"{giver} wrote it down here.")
            if recording_kept:
...
class ConsentRecord:
    ...
    rendered: Rendered
    region: Region
    """The region the record belongs to. A label for the channel, which guards the leaving."""
...
class _Voice:
    ...
    def opens(self, name: str) -> str:
        return "open" if self.who(name) == "you" else "opens"
...
def _lines_of(wording: str) -> list[str]:
    """The words as read, one idea per line: the row's lines, and the sentences in each."""
    return [
        sentence
        ...
        for title, entries in by_title.items():
            ...
            for entry in entries:
                ...
                for line in basis_lines(
                    ...
                    patient=me,
                    patients=mine,
                    witness=voice.who(witness) if witness else None,
                    recording_kept=entry["recording_kept"],
                ):
                ...
                if entry["wording"]:
                    ...
                    lines.append(f"  Nura did not keep the words {giver} read that day.")
                if entry["status"] == "withdrawn":
                    if entry["withdrawn_by"]:
                        ...
                        lines.append("  Nura does not know who stopped it.")
                elif entry["status"] == "out_of_date":
                    lines.append(f"  Nura has changed these words since {giver} said yes.")
                    lines.append(
                        cap(
                            f"  Nura will ask {me} to say yes again the next time {me} "
                            f"{voice.opens(patient)} the app."
                        )
                    )
                else:
    ...
    *,
    ...
    for consent in consents:
        ...
        entry: dict[str, Any] = {
            ...
            # In the language he read the words in, so the page names the parts as he knows them.
            "what_lines": what_lines(scopes or (), consent.language),
            ...
            "witness": (
                await name_of(consent.witness_person_id)
                if consent.witness_person_id is not None
                else None
            ),
            # Yes or no, never the artefact's id: that id opens the recording itself.
            "recording_kept": consent.basis_artifact_id is not None,
            "given_by": giver,
