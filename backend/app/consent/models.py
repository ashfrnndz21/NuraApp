...
the record can show which words were agreed to each time. The row holds no health content:
it says what was agreed to, never what the graph says.
"""
...
class ConsentBasis(StrEnum):
    ...
    someone else agrees for him: a lasting power of attorney, a doctor's letter, or his own
    spoken agreement recorded by a witness.
    """
...
class Consent(ProfileScoped, Base):
    ...
    `person_id` is who gave it — the owner, or the steward or chief acting for him on the
    recorded `basis`. `text_version` names the wording they saw (see `app.consent.texts`),
    `language` the language they saw it in.
    """
