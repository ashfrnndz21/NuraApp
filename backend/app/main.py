...
summariser is the fixture one over NURA_VISIT_FIXTURES until a model in the region does (E05);
the WhatsApp provider is the fixture (`NURA_WHATSAPP_PROVIDER=fixture`, signing with
`NURA_WHATSAPP_DEV_SECRET`), which also only runs on a declared dev run (E19). Logging is set
up so that, on a dev run, the code line is seen.
"""
