...
from sqlalchemy import JSON, CheckConstraint, ForeignKey, Integer, UniqueConstraint, event, inspect
...
class Dimension(StrEnum):
    """The six dimensions. Every one of them is computed, every time."""
...
class Posture(StrEnum):
    """How the day is held: the wash on the screen and the order of the feed, in one word.
...
class StateTrigger(StrEnum):
    """What made State recompute. Every snapshot records one."""
...
FACT_TRIGGERS = frozenset({StateTrigger.NEW_FACT})
"""The triggers that must name the fact they came from, in the table as well as the service."""


class ImmutableState(Refusal):
    """A snapshot is what was known then. It is superseded, never edited."""
...
class StateSnapshot(ProfileScoped, Base):
    ...
    highest one and two writers cannot both believe they wrote it. `stale_after` is the
    moment a window in it closes — a visit coming inside the week, a fact that stops
    holding — which is how time alone is enough to make State recompute.
    """

    __tablename__ = "state_snapshot"
    __table_args__ = (
        UniqueConstraint("profile_id", "sequence", name="uq_state_snapshot_sequence"),
        # A snapshot that says a fact caused it must name the fact. Same discipline as
        # provenance on the fact itself: nothing here is asserted from nowhere.
        CheckConstraint(
    ...
    # Why the posture is what it is: the fact ids and episode ids that raised it. The one
    # thing State works out rather than repeats, so it carries provenance like anything else.
    posture_because: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
    trigger_kind: Mapped[StateTrigger] = mapped_column(enum_column(StateTrigger, "state_trigger"))
    ...
    # The newest fact this snapshot folded in, or None when it folded none. A snapshot that
    # folded nothing is behind the moment any fact exists at all, which is what catches a
    # fact asserted at the very instant the snapshot was computed.
    folded_through: Mapped[datetime | None] = mapped_column(default=None)
...
def _refuse_edits(mapper: Any, connection: Any, target: Any) -> None:
    if any(attribute.history.has_changes() for attribute in inspect(target).attrs):
        raise ImmutableState("a state snapshot is superseded, never edited")
...
def _a_card_names_its_state(mapper: Any, connection: Any, target: Any) -> None:
    ...
    if isinstance(target, RenderedFromState) and getattr(target, "state_id", None) is None:
        raise NotRenderedFromState(
            f"{type(target).__name__} is rendered from a state snapshot, and named none"
        )
