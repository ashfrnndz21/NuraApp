...
def checkpoint_8(client: httpx.Client) -> None:
    ...
    if types[:3] != ["now", "reading", "gate"]:
        raise fail("Pa opens his feed", why=f"expected now, reading, gate first; got {types}")
    if not types[3:] or not set(types[3:]) <= {"story", "learning"}:
...
CHECKPOINTS = {2: checkpoint_2, 3: checkpoint_3, 4: checkpoint_4, 5: checkpoint_5, 8: checkpoint_8}
