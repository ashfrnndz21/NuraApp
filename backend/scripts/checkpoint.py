...
def checkpoint_9(client: httpx.Client) -> None:
    ...
    ok(
        ...
        "his State and today's doses, through his WHATSAPP consent, verified against plain words:"
    )
    ...
    if "safety_flag" not in steps or "safety_escalation" not in steps:
        raise fail("Pa reads the emergency lines of his trail", why=f"got {steps}")
    if steps.index("safety_flag") > steps.index("safety_escalation"):
    ...
    ok(
        ...
        "— the Flag was written first, before the message was even kept, then the message, then the "
    ...
    for row in reversed(trail):
        if row["action"] == "write" and row["target"] in ("safety_flag", "safety_escalation"):
