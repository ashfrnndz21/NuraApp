...
from tests.paper import LIPID_PANEL, WARFARIN_LABEL
...
async def test_the_sitting_walks_its_steps_and_reads_the_papers_back(
    ...
    assert opened["prompt"]["lines"][0] == "Beritahu kami bahasa anda dan cara yang anda suka."
    ...
    assert ready["prompt"]["headline"] == "Ini yang kami faham"
...
async def test_a_shoebox_of_30_papers_becomes_the_record_in_one_sitting(
    ...
    assert closed["summary"]["lines"][0] == "Kami simpan 30 kertas anda."
...
async def test_every_read_back_line_is_served_in_his_language(
    ...
    if language == "en":
        ...
        assert view["read_back"][10]["line"] == "The label says 1 tablet once a day at night."
