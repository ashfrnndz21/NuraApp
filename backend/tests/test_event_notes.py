...
is a VOICE artefact — resting on the recording consent (E16-02) — heard by the region's
transcriber, its words kept by reference and never a fact; it can be heard again. A scribble
...
async def test_a_voice_note_rests_on_the_recording_consent(deployment: Deployment) -> None:
    pa, profile_id, event_id = await _reading(deployment)
    refused = await deployment.client.post(
        _notes(profile_id, event_id), json=voice(AFTER_THE_WALK), headers=bearer(pa["token"])
    )
    assert refused.status_code == 403 and refused.json()["refusal"] == "ConsentWithheld"
    assert "ConsentWithheld" in await refusals(deployment, pa, profile_id)
    listed = await deployment.client.get(_notes(profile_id, event_id), headers=bearer(pa["token"]))
    assert listed.json() == []
