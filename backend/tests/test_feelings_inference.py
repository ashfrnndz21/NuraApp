...
from app.safety.boundary import Surface, boundary_line, boundary_lines, is_boundary_line
from app.safety.red_flags import Escalation, Feeling, Flag
from tests.family_support import Household, household
from tests.feelings_support import REGISTRY, blood_pressure, happened, new_medicine, visit_with
...
async def test_a_tap_asks_when_it_began_and_is_read_against_a_new_medicine(
    ...
    tapped = await record_tap(sg, context=owner, word=Feeling.DIZZY, registry=REGISTRY)
...
async def test_a_helpers_key_starts_the_red_path_and_nothing_else(sg: AsyncSession) -> None:
    ...
    tapped = await record_tap(sg, context=siti, word=Feeling.FALL, registry=REGISTRY)
    ...
    async with refused_unit(sg, OutOfScope):
        await record_tap(sg, context=siti, word=Feeling.TIRED, registry=REGISTRY)
