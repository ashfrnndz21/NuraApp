...
- **Nothing uploads until Stop.** (Superseded by ADR 0013: chunks go as it records, and nothing is kept until the doctor's yes.) The audio is held in the page's memory as the recorder hands it over, and is sent once, as the request body of `POST …/appointments/{appt}/recording`. If the upload fails the screen says so and keeps the audio, so **Stop** can send it again. Nothing is written to the phone's storage.

### The clip: the whole artefact, with the stretch to play

A citation of a consult carries `start_s` and `end_s`, from the speaker segments the item's words fall in. A byte range of a webm or an mp4 is not a playable file: the header and the cues are at the front, and the clusters do not line up with seconds. Cutting a real clip needs a demuxer, which is a new dependency. So `GET /profiles/{id}/artifacts/{a}/clip?start=&end=` answers with the whole artefact, read under the scope it was written under — the visits', where a consult and its transcript are kept since row scope (ADR 0004), and where the recording is found — with the stretch in `X-Clip-Start`, `X-Clip-End` and `X-Media-Fragment: t=start,end`. The phone plays it as a media fragment, sets the start itself when the fragment is ignored, and pauses at the end. This is the honest choice: anyone who can hear the clip can already hear the whole recording (the same door — him and the family he let in), so nothing is disclosed that the door did not already open. What is disclosed is exactly what the trail says was read.
...
- **Who can hear a recording** is the patient and the family he let in — his chief and his caregivers — and nobody else, whatever scope a key holds. That is what the room is told ("Only you and the family you let in can hear it."), what the printed card says and what the consent's words say; the rule is in one place, the artefact door (`app/memory/episodic.py`: `require_artifact` and `require_artifact_under` refuse a consult, `OnlyTheFamilyHears`, 403, on his trail), so the clip, the whole recording and the transcript heard from it cannot be reached another way. A viewer or a clinic key reads the visit and its card. The person who records needs the records scope as well, because every artefact is kept through that door (`store_artifact`, `may_record`). Counsel's sign-off (recording-consent.md §8) stays.
- **Recall cites only what he confirmed.** Before the post-visit card has his yes, an answer about the visit says the card is waiting for it and cites the card, not the recording; after it, a confirmed item is cited with its place in the recording.
- Answered by ADR 0013 (#129): the recording now goes up in chunks as it records and resumes after a dropped connection; nothing is kept until the doctor's yes, and a no throws away what was sent. Still open: how long a recording is kept once its card is read (pdpa-data-map §4, question 4 for counsel).
