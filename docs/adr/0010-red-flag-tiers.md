...
Every reply ends "Nura does not decide what is wrong." The family's notice follows the same step: pending templates `red_flag_notice_ambulance`, `_hospital` and `_night`. Until Meta approves them they go as free text inside the family member's 24-hour window; outside it, once a tier applies, WhatsApp says only the tier's own words or the neutral `red_flag_notice_urgent` ("This one we do not wait for. {name} is not feeling well. Open Nura now."), never the approved "Call {doctor} today." — an urgent alert is never told at a lower tier than the one it is (#174). Where even the neutral notice is not approved, no WhatsApp goes at all and the delivery's own trail says why; the app push and the family page's notice still reach the family. A flag never waits on Meta, but it never speaks in a weaker voice either.
...
- The family's tiered notices are pending Meta. **Revised (#174):** the approved "Call {doctor} today" notice is never a fallback for a tiered flag — it would tell the family a lower urgency than the tier. The last resort on WhatsApp is the neutral `red_flag_notice_urgent`, itself pending; where that is not approved either, no WhatsApp goes and the reason is on the trail, while the app push and the family page's notice still reach the family. The switch must not be set in a deployment until `red_flag_notice_ambulance`, `_hospital`, `_night` and `_urgent` are approved; that is a condition of the sign-off (`docs/trust/clinical-sign-off.md`). Until then, with the switch unset, every flag's notice is the ambulance one inside the window, and outside it either the ambulance notice or, once its own template and the neutral one are both unapproved, no WhatsApp at all — never "Call Dr Tan today," at any hour, for a tier this ADR marks as urgent.
