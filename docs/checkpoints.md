...
- **The day's nudge** (E11-07): on a fresh Pa with no tablet (steps 1 only), Today has the day's nudge from 10:00 on his wall — its lines, its why ("You see this because …") under them, *OK* and *Not today*. *OK*: it is gone, and `GET /profiles/{id}/nudges` shows it answered. While the feeling cloud is on Today, a check-in nudge is not shown: the cloud is the check-in.
