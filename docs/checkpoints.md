...
|---|------|--------------|-------------------|--------|
| 1 | Heartbeat | `make dev`, open http://127.0.0.1:8000/docs, call `GET /health` | scaffold | **passed (2026-09-18, fresh database)** |
| 2 | Accounts, keys, consent, audit | Register by phone code (the code prints in the server log, no SMS), create a profile, issue a caregiver key scoped to `medicines, visits`, watch it read medicines and get refused on notes, then list the refusal in the audit trail | E00-01, E00-02, E00-07 | **passed (2026-09-18, fresh database)** |
| 3 | Memory and State | Add a blood-pressure reading as a Fact with provenance, watch State recompute and record the trigger; a Fact without provenance is rejected; a key without the record cannot read State | E00-03, E00-04 | **passed (2026-09-18, fresh database)** |
| 4 | Three doors, proxy, claim | Set up a profile *for someone* by phone number, try to create a second one for the same number (refused, in words that name nobody), then claim it as the patient and see the steward key become the chief key with consent recorded | E01-01, E01-02 | **passed (2026-09-18, fresh database)** |
| 5 | Paper in, facts out | Upload the sample lipid report from `backend/tests/fixtures/paper/`, get a review card with per-field confidence (the unsure fields dotted), correct one value, confirm with one OK, and see Facts with provenance appear; upload the warfarin label and see the high-risk class on the card; a key without the record cannot see a card | E02-01, E02-07, E16-04 | **passed (2026-09-18, fresh database)** |
| 6 | Medicines | Add three drugs from a label, see the reconciliation (refill vs dose change), the interaction check, the running count and reorder date, and the medication story in plain words; a high-risk drug refuses a dose without a label photo | E04-01…E04-07, E16-04 | **passed (2026-09-18, fresh database)** |
| 7 | Plain words and the visit loop | Add three medicines through E04 and book a visit with Dr Tan; read the pre-visit brief in Malay (every line passes the plain-words verifier, ending on the boundary line) and see it as the visit card on the feed; the questions card (three lines and the boundary); paste the visit transcript and get the summary card with the dose change as a question for the doctor; confirm it and see the memos (and the memo card on the feed), the planned follow-up and the facts citing the transcript; paste a red-flag transcript and see the card carry the flag and "Telefon Dr Tan hari ini."; see a fragment refused | E22-01, E04, E05-01, E05-02, E05-05, E05-06 | **passed (2026-09-18, fresh database)** |
| 8 | Feed (backend) | Call the feed endpoint and see the supply order (now, today, gate, story, learning) with why-am-I-seeing-this on every card; page twice with the cursor; a burst of readings is capped; quiet hours hold everything but a red flag, which jumps the queue; "Not for me" holds that kind of card for the day; Mei sees the caregiver supply; a learning card from an allowlisted source appears after the self-search job runs; a medicine running low makes a reorder card from E04's count | E21 backend (Session 8) | **passed (2026-09-18, fresh database)** |
| 9 | WhatsApp (sandbox) | Mei forwards a photo to the number and it files itself as a review card and replies; she posts "BP 150/90" and gets a read-back that only her own "yes" turns into a Fact; a stranger's number gets one fixed line and nothing is stored; "he fell" writes a Flag first and escalates in-thread; the morning card goes to Pa as an approved template and his "tired" is written down; the thread is by reference and every line is on the trail | E19-01…E19-03, E19-05 | **passed (2026-09-18, fresh database)** |
...
| 28 | The feed, full (web) | On Pa's feed, a clip card: a still from Nura's own server (no video site is asked), *Play* on a tap and never by itself, the line being said under the still, and "Watch the whole video on National Heart Centre Singapore", a link he taps; *His week, in 30 seconds* over the same still; in Penang with his town set on his yes, "More dengue near you this week" with what to do today and the boundary last — and nothing at all for a profile no condition makes it relevant to; a food card with one choice for the week; Mei on Home: *Watching for Pa* (each watch, its sources, how often; pause one, add festive food), and Ramadan only on Pa's own yes and *Sent to Pa this week* (each card with what became of it, where it came from, *Hear*, *Stop watching for this for now*); the ask bar's *Your papers / Web / Doctors and clinics / Videos*, the web and videos from the allowlist only; a card made while he reads lands under the card on screen, never lost | E09-06, E09-07, E11-08, E11-09, E21 (F1, ADR 0001) | **ready** |
...
- **Backend checkpoints (1–9, 13–18, 20–23)**: `make dev` in one terminal, `make checkpoint N=<n>` in another. The script runs the scenario against the local server with a fixture provider (no SMS, no real drug database, no WhatsApp) and prints each step with ✓ or ✗; it stops at the first ✗. The FastAPI page at `/docs` lets you repeat any step by hand. `make dev` also writes its log to `backend/.dev.log` (ignored by git), which is where the script reads the login codes from; `make reset-db` gives you a clean local database (stop `make dev` first).
- **Web checkpoints (10–12, 24, 26–28, ADR 0001)**: `make dev` and `make web` in two terminals, then the app in a browser — on the Mac at http://127.0.0.1:5173, on the phone at the Mac's address on the same Wi-Fi. The operator walks it first with Playwright (`make web-e2e`) and attaches screenshots to the checkpoint note; you then walk it yourself by hand.
...
## Fresh database, 2026-09-18

Proved that the whole backend replays from nothing: a brand-new SQLite file, `alembic upgrade
heads` from empty, migration chain 0001 → **0046_insurance_policies_claims** applied cleanly,
then every checkpoint `backend/scripts/checkpoint.py` knows how to run, walked in order
against it over HTTP. Not run: the web checkpoints (10–12, 24, 26–28 — need `make web` and a
browser), CP19 (needs a hosting account) and the unnumbered TestFlight row (needs an Apple
developer account) — unchanged, per "How a checkpoint is tested" above.

| Checkpoint | Result |
|---|---|
| 1 | ✓ passed (the server answers `GET /health`, proven at the top of every other run below) |
| 2 | ✓ passed |
| 3 | ✓ passed |
| 4 | ✓ passed |
| 5 | ✓ passed |
| 6 | ✓ passed |
| 7 | ✓ passed |
| 8 | ✓ passed (after a runner fix, see below) |
| 9 | ✓ passed |
| 13 | ✓ passed (after a runner fix, see below) |
| 14 | ✓ passed |
| 15 | ✓ passed |
| 16 | ✓ passed (after a runner fix, see below) |
| 17 | ✓ passed |
| 18 | ✓ passed (after a runner fix, see below) |
| 20 | ✓ passed (its own server, `NURA_FROZEN_CLOCK=2026-09-14T06:00:00+08:00`, as this checkpoint's own how-to says) |
| 21 | ✓ passed (after a runner fix, see below) |
| 22 | ✓ passed (after a runner fix, see below) |
| 23 | ✓ passed (after a runner fix, see below) |

18 of 18 runnable checkpoints passed. Every failure on the first pass was the runner's own
`backend/scripts/checkpoint.py`/`backend/scripts/checkpoints/cp*.py` falling behind a
deliberate change made this week, never a regression in the app:

- **The consent role and window are now mandatory** on `POST /profiles/{id}/consents/sharing`
  and its `/preview` (#185, `SharingConsentIn`/`SharingPreviewIn`, `app/channels/api/schemas.py`).
  The runner was still posting the old, shorter body. Fixed in `checkpoint.py` (checkpoints
  2, 3, 5, 6, 7, 8, 9) and in `checkpoints/cp13.py`, `cp14.py`, `cp15.py`, `cp16.py`, `cp17.py`,
  `cp18.py`, `cp20.py`, `cp21.py`, `cp22.py`, `cp23.py` — every `consents/sharing` call now
  names the role and window the paired `keys` call actually cuts.
- **A treatment-changing finding from the self-search engine is rerouted as a real question
  for the doctor**, never a `questions` `FeedItem` (#236, `app/delivery/feed/search.py`,
  `_ask_the_doctor`): checkpoint 8's explainer job now shows an empty `questions` list and a
  `rejected` entry named `treatment_change_rerouted_as_question` with a real `memo_id`. Fixed
  the assertion in `checkpoint.py`'s `checkpoint_8`.
- **`FeelingNote.lines` anchors "since yesterday" to the day he answered** rather than saying
  it live (PR #233 review, `TELL_ON` vs `TELL` in `app/reasoning/feelings/strings.py`): a note
  read back later must never go stale or false. Checkpoint 21 expected the old, live phrase
  ("Beritahu Dr Tan bahawa anda rasa pening sejak semalam."); fixed `checkpoints/cp21.py` to
  compute the day-anchored phrase the same way `compose_note` does.

No real regression was found in the app itself.

**The demo seed, on a second fresh database.** A second brand-new SQLite file, migrated the
same way, started with `NURA_DEMO_SEED=1 NURA_DEV_CODE_SENDER=1`: the seed ran once at startup
(`nura.demo_seed: demo: seeding Pa (+6500001111) and Mei (+6500002222)`), and `POST
/dev/quick-signin {"as": "pa"}` ("Try it as Pa") opened onto a living record — 4 medicines
(amlodipine, atorvastatin, metformin, warfarin, the last high-risk and resting on a label
photo), blood-pressure readings, a past and a future visit with Dr Tan, and Mei holding a
chief key — never an empty one. A second start of the same server, same database, logged no
second seed line and left every id, count and key exactly as the first start had: the
profile id, the 4 medicines and the 1 key were unchanged, proving `seed_demo`'s own claim to
be idempotent.
...
✓ a first try, as the Visit screen sends it (#129): the upload opened as the microphone opened; a chunk sent before Dr Tan answered was refused (409 NoYesFromTheDoctor), so nothing left the phone; Dr Tan said no — DELETE …/uploads/{upload}?because=no closed the upload, and no recording was kept (GET …/recordings is [])
✓ Mei's recording, in 3 chunks as it was made (POST …/recording/uploads, audio/webm;codecs=opus), Dr Tan's yes before the first (POST …/yes); chunk 1 sent again after its answer was lost changed nothing, and GET …/uploads/{upload} said where to go on from (2 chunks, 32 bytes). On Stop (POST …/finish, 66 s) the chunks were put together in the region into one consult voice artefact 3c394484… on the RECORDING consent e77a7234…, heard at 0.93, and who spoke when — 11 stretches, no words in any row:
      0.0–8.6   unknown
...
2. **Dr Tan said yes** keeps listening. The audio goes up in chunks while Nura listens (#129). **Stop** sends the rest, and the server puts the chunks together. The screen says *Nura kept the recording.* and shows the post-visit card. Under each line Dr Tan said is *Hear what Dr Tan said*, which plays only that stretch.
3. **Dr Tan said no** stops the microphone and throws the audio away on the phone, and every chunk already sent is thrown away on the server. Nothing is kept. The screen says *Nura will not listen today. You will write the notes by hand.* and offers a box for the notes, read into the same card.
...
5. **Lose the connection while it listens** (flight mode on, then off): the screen says *The phone has no connection right now. Nura sends the recording when the connection is back.* and keeps listening. When the connection is back it asks the server how far it got and sends from there. A Stop with no connection keeps the recording on the phone and sends it the moment the connection is back (ADR 0013).
...
```
✓ tests/e2e/visit.spec.ts:63:1 › the visit screen: logistics from the record, the yes to a driver, consent, the notice first, chunks as it listens, kept on Stop, the card with its clips
✓ tests/e2e/visit.spec.ts:186:1 › a no keeps nothing: the recorder stops, what was sent is thrown away, and the notes are written by hand
✓ tests/e2e/visit.spec.ts:219:1 › the page hidden while listening stops at once, and what was heard is kept on the phone for one tap
✓ tests/e2e/visit.spec.ts:248:1 › a dropped connection while Nura listens: it says so, sends the rest when it is back, and Stop keeps it all
```
...
✓ 10:05, after his check-in time (10:00): the schedule handed the day's nudge over and delivery sent it, so the plain check-in — held once a nudge already asked how he is — sends nothing of its own — Pa (patient): sent by whatsapp, template nudge; rule nudge_handed_over; the web handing it over again as he answers (POST /nudges/plan) gives back the same nudge — one row (GET /nudges) — and 10:10 sends it no second time. What he reads:
...
✓ every attempt is on the delivery log with the rule that fired: breakfast_anchor_reached, dose_window_closed_untapped, evening_family_notice, nudge_handed_over, paper_waiting_for_a_yes, red_flag_raised, reorder_date_reached
...
2. **His check-in time.** As Pa, `PUT /profiles/{profile_id}/settings` with `{"language": "en", "checkin_time": "18:00"}`; stand the clock at 18:00 the next day (`POST /dev/clock`) and run the engine (`POST /dev/run-triggers`): the check-in goes to him then, and not at 10:00. His "OK" to it (`POST /dev/whatsapp/inbound`) is written down as how he feels. A whole day on WhatsApp alone, driven only through these two routes, is `backend/tests/test_whatsapp_day.py`.
3. **Hear a card.** `GET /profiles/{profile_id}/feed/{item_id}/voice` for any card on his feed: `audio/wav`, `X-Duration-Seconds` under 30, `X-Voice-Cache: hit` the first time already — a card's audio is said and kept when the card is made, under its voice script's digest (E22-03), so the first play is a read; `?language=ta` is a 404 (Tamil comes at T2) and the web client says it with the phone's voice.
...
## How to run checkpoint 24

The web client's polish (W4, ADR 0001's substitutes): the one player (E15-07); the offline copy, taps held offline and the emergency card on the phone (E00-08, the web half of E13-01, ADR 0010); papers from photos (E18-01: a browser cannot scan the photo library, so the phone's own picker takes many at once); and accessibility (E15-04: screen-reader semantics and the browser's text size in place of VoiceOver and Dynamic Type). The same terminals as checkpoint 10; it depends on no other checkpoint.

```sh
make dev                # terminal 1: the API on http://127.0.0.1:8000, log to backend/.dev.log
make build-web          # the built app, which make dev serves at http://127.0.0.1:8000/app/ (the offline part)
make web                # terminal 2, for the phone: http://<Mac's address>:5173/app/ (proxies /api)
printf '\x89PNG\r\n\x1a\nnura-paper-placeholder:lipid-panel-2023-09-07\n' > ~/Desktop/lipids.png
printf '\x89PNG\r\n\x1a\nnura-paper-placeholder:receipt-2026-09-01\n' > ~/Desktop/receipt.png
printf '%%PDF-1.4\nnura-paper-placeholder:discharge-letter-2026-08-20\n' > ~/Desktop/letter.pdf
                        # redacted papers the fixture extractor knows, as the bytes it knows them by
```

Sign in with a fresh number as in checkpoint 10 (the code is in the `make dev` terminal), *This is for me*, *I agree*, *Set up later*, and give yourself a medicine with four doses a day the way checkpoint 10 does at http://127.0.0.1:8000/docs (`"dose_text": "1 tab QDS"`), so Today has a Now card between 05:00 and 11:00. AirDrop the three files from the Desktop to the iPhone and *Save to Files*: they are not real pictures, so Photos will not take them.

**On your iPhone** (the same Wi-Fi, `http://<Mac's address>:5173/app/`, as checkpoint 10):

1. **The one player.** On Today, tap *Hear* under the number that only goes up. One player opens under it: a big *Pause*, *Slower*, *Usual speed* and *Faster*, and the line being said, in the same big writing as the card. *Pause*, then *Play*: it goes on from where it was. Tap *Slower*, close the app and open it again: *Hear* now reads slower — the phone kept it. Tap *Me*: the voice stops and the player goes. Nothing speaks until you tap. Today's cards are read by the phone's own voice; a card under *See more for you* plays Nura's own voice for it (E11), which on a dev run is the fixture — silence as long as the card — so the player runs there and you hear nothing until the real voice exists.
2. **The emergency card.** Tap *Your emergency card* near the bottom of Today (it is under Me too). It shows the backend's lines and no others — *This is Pa's emergency card.*, what you take, whom to call — the ambulance's number as a button that dials (*Call the ambulance on 995*), a big *Print this card* that opens the backend's printable page in a new tab (Share, then Print), and the day Nura last read it.
3. **Papers from your photos.** Me → *Add papers from your photos* → *Choose photos* → *Choose Files*, select the three files, *Open*. The grid shows *Paper 1* to *Paper 3*, each *Nura will send this one.*; tap one to leave it out (*Nura will not send this one.*). Nothing leaves the phone until *Send 2 papers*. Then each paper says what became of it: the blood test with *Check this paper* (the same review card as onboarding; only its *Looks right* writes anything), the receipt in the backend's own words that it is not a health paper, and *Nura kept no photo on this phone.* In onboarding, *Choose many photos* on *Now, your papers* does the same inside the sitting. A real photo from your library is not one of the fixture papers, so until the real extractor exists (E02) it comes back as a page Nura could not read — that is the fixture, not the screen.
4. **Twice the text, and a screen reader.** In Safari, tap *aA* and the bigger *A* until it says 200%. Every screen grows: rows of buttons wrap onto a second line, nothing runs off the side, nothing sits on top of a line. With VoiceOver on (Settings → Accessibility → VoiceOver), each new screen starts at its heading, every button says what it does, and the read-back says *Nura will keep that.* after *Yes, that is right*. In onboarding, a *Yes* to *Would bigger writing help you?* makes the writing one step bigger on your own phone (your State's large-text setting), and never on your family's.

**On the Mac, with the network off.** Over plain http on a LAN address the iPhone will not install the offline worker (that needs the https deployment, checkpoint 19); on the Mac `127.0.0.1` counts as secure, so this part is checked there, at http://127.0.0.1:8000/app/ with the built app.

5. **Taken, held.** Open Today while a dose is due and turn the network off (Wi-Fi, or Safari's *Develop → Network Conditions → Offline*). Tap *Taken*: the card says *You tapped this at 8.05 in the morning.* and *Nura will send what you tapped when the internet is back.*, and the next dose due, if there is one, becomes the Now card. Reload: still held. Turn the network on: *Nura sent what you tapped.*, and the number goes up by one. At /docs, `GET /profiles/{id}/medicines/today` shows the dose taken once; `POST …/taken` with the same `taken_at` twice writes it once, and a `taken_at` from yesterday is refused (`TapNotToday`).
6. **The card after midnight.** With the network off, *Your emergency card* opens with its lines and the day it was read, and *Print this card* still opens the page. The next morning, still offline — or at once, by deleting `today.<profile id>` in Safari's *Develop → Show Web Inspector → Storage → IndexedDB → nura → kv* — Today shows *Nura cannot reach your papers right now.* and the emergency card, and nothing else. Sign out and none of it is left on the phone.
7. **A neighbour's key.** At /docs, as yourself: `POST /profiles/{id}/consents/sharing` with `{"holder_phone_e164": "<her number>", "holder_display_name": "Lin", "scopes": ["emergency"], "relationship": "neighbour", "language": "en", "captured_via": "app"}`, then `POST /profiles/{id}/keys` with `{"holder_phone_e164": "<her number>", "role": "emergency", "scopes": ["emergency"]}`. Sign out and sign in as her number: her door opens your emergency card and nothing else — no Today, no feed, nothing under Me that opens more.

What you will see (the operator's walk: `make build-web`, then `make web-e2e`, which starts `make dev` with both clocks frozen at 10:00 in Singapore):

```
✓ player.spec.ts    › Hear opens the one player: nothing before the tap, Play and Pause, his speed kept on the phone, the line being said in his body size
✓ player.spec.ts    › a clip in the player: only its stretch, its line as the transcript, Pause and Play
✓ player.spec.ts    › a recording this key may not hear: the backend's refusal said on the line, and no player
✓ offline.spec.ts   › Taken with no network: held with the moment he tapped, then sent once each, in order, when the network is back
✓ offline.spec.ts   › a no to a held tap is said in the backend's words, and nothing of the papers stays
✓ emergency.spec.ts › the emergency card, one tap from Today and from Me: the backend's lines only, Print opens its printable page, readable with no network and still there past midnight
✓ emergency.spec.ts › her density: the chief reads the same card, one tap from Today
✓ emergency.spec.ts › a neighbour's key to the emergency card alone opens that card and nothing else
✓ emergency.spec.ts › on an iPhone in Safari, before Nura is on the home screen › the emergency card says how to add Nura to the home screen
✓ budget.spec.ts    › cold start on a slow phone at 360 by 640: his list and the emergency card in under 3 seconds, with no network and on a slow one
✓ papers.spec.ts    › papers from his photos: many at once, a grid he confirms, nothing sent before his yes, a review card each, a page that is not a health paper said so, no photo left on the phone
✓ papers.spec.ts    › two years of photos: 240 picked at once are a grid in well under a minute on a slow phone, and nothing is sent
✓ papers.spec.ts    › in the sitting: many photos at once, and each paper he checks joins the sitting
✓ a11y.spec.ts      › every screen in the patient density: no serious or critical axe finding
✓ a11y.spec.ts      › every screen in the caregiver density: no serious or critical axe finding
✓ a11y.spec.ts      › every screen in the patient density, under the demo banner: no serious or critical axe finding
✓ a11y.spec.ts      › the writing at 200%, on a 360 px phone: nothing lost, nothing sideways, nothing drawn over a line
✓ a11y.spec.ts      › the writing at 200%, on a 360 px phone, under the demo banner: nothing lost, nothing sideways, nothing drawn over a line
✓ a11y.spec.ts      › Tab goes through what can be pressed in the order the eye reads, each with a ring; a new screen starts at its heading
✓ a11y.spec.ts      › Reduce Motion: nothing moves that he did not ask for, and what answers a tap does so at once
✓ a11y.spec.ts      › his large-text setting, from his State, makes the writing one step bigger on his own phone and on nobody else's
```

The same suite walks a demo deployment (ADR 0008) with `NURA_E2E_DEMO_CODE=<the demo's code>`: numbers in the test range and the demo's code in place of the log's. A demo refuses a frozen clock, so the tests that stand the backend at 10:00 (the dose windows, the quiet hours) are for the dev run.

**What "passed" means.** Nothing spoke until you tapped *Hear*; one player at a time, with *Pause* and *Play*, your speed kept on the phone and the line being said in your own body size; a visit's clip played only its stretch, and a recording your key may not hear said so in one plain sentence and showed no player. *Taken* with no network was kept with the moment you tapped, said as held, and written once, at that moment, when the network came back — and a no to it was said in one plain sentence with nothing of the papers left behind. The emergency card was the backend's lines and nothing else, one tap from Today and from Me, printable, readable with no network, still there after midnight with the day it was read, gone at sign-out, and all a neighbour's key could see. Your photos went only on your yes, each came back as a review card or in the backend's words as not a health paper, and none was kept on the phone. Every screen read in both looks with nothing serious for a screen reader, at twice the text with nothing lost, sideways or drawn over a line, in the order the eye reads, and still under Reduce Motion. If a step does not do that, tell the operator which one and what you saw instead.
...
## How to run checkpoint 28

Checkpoint 28 is the feed's richer formats in the web client (F1): compressed video as clip cards, his week in 30 seconds, local and seasonal alerts, the week's food card, what he did with his cards going back to Nura, and the chief's two panels on Home. Every card line, every why and every source is the backend's; the screens only lay them out, and nothing plays until it is tapped. The operator's walk is `make web-e2e` (`web/tests/e2e/formats.spec.ts`, and `day.spec.ts` for the card made while he reads); this is the same walk by hand.

**Set up, on the Mac.** Start over, build the app, and start the backend with its clock stood at ten in the morning on Monday 14 September:

```sh
make reset-db                                               # a clean local database
make build-web                                              # the app, served by the backend
NURA_FROZEN_CLOCK=2026-09-14T10:00:00+08:00 make dev        # terminal 1: http://127.0.0.1:8000/app/
```

1. **Pa.** Open http://127.0.0.1:8000/app/, sign in with a fresh `+65` number and the name *Pa*, *This is for me*, *I agree*, *Set up later*. At http://127.0.0.1:8000/docs as Pa (*Authorize* with his token), write down two blood pressures from earlier in the week (`POST /profiles/{id}/readings` with `taken_at` on Monday 7 and Friday 11 September) and one today, `138` over `84`.
2. **Mei, his chief.** In a private window sign in with another fresh number and the name *Mei*, then, as Pa at `/docs`, let her in (`POST /profiles/{id}/consents/sharing` with every part) and cut her a key (`POST /profiles/{id}/keys` `{"holder_phone_e164": …, "role": "chief"}`), as checkpoint 27 step 2 does. In Mei's window, *Me* → *Smaller, with more on the page*.

**On the Mac, what you will see.**

- **A clip** (E09-06): on Pa's Today, *See more for you*, and page down past the gate to "Your blood pressure, in 30 seconds". Under its lines is a still with *Play*. In the browser's developer tools (*Network*), the still and the captions came from `127.0.0.1` — `…/feed/{item}/clip/poster` and `…/clip/captions` — and nothing went to a video site. Nothing plays when the card arrives. Tap **Play**: the card is read out, and the line being said shows under the still, in his language. "Watch the whole video on National Heart Centre Singapore" opens the heart centre's own page in a new tab, only when tapped. The publisher's licence does not allow a copy, so Nura keeps none: `GET …/clip/video` answers "This one is a picture with a voice." *Hear* and *Not for me* are on the card, as on every card.
- **His week in 30 seconds** (E11-09): further down, "Your week, in 30 seconds": "This is your week, from your blood pressure book." and his own numbers from this week, over the same still, *Play* on a tap. It repeats his book; there is no boundary line and no link.
- **What he did goes back** (E11-08): the card at rest on his screen, *Play*, *Ask*: at `/docs`, `GET /profiles/{id}/feed/week` as Mei shows that card as `opened`, and after *Play* as `played`. In *Network*, one `POST …/feed/events` carries what he did — `opened` without seconds, `played` with the seconds of the voice — and nothing measures how long a card was on his screen.
- **The chief's Home** (spec §1): in Mei's window, Today in the smaller look has **Watching for Pa** — "Blood pressure, in simple words", "Amlodipine, in simple words" once a tablet is on his list, each with its sources and how often — and **Sent to Pa this week**, each card with what became of it ("Pa opened this card.", "Pa heard this card.", "Nura kept this back from Pa."), where it came from, *Hear*, and *Stop watching for this for now* on a card a watch found. *Watch for something new* → *Festive food*: "Festive food, before it comes" is on the list, weekly. Ramadan is not among her choices: whether he fasts is his to say. *Pause* on a watch: it says *Paused*, and *Start again* undoes it. No count of anything anywhere.
- **The ask bar's filters**: *Ask or search* on Today. In Mei's smaller look it has *Your papers*, *Web*, *Doctors and clinics* and *Videos*. *Web*, "blood pressure", *Search*: pages from HealthHub and the National Heart Centre only, each said in plain words with the boundary last and "Read the page on HealthHub"; *Videos*: the heart centre's video only; *Doctors and clinics*, "tan": Dr Tan from his own list. In Pa's look there is one thing: his records.
- **His town** (E09-07): Pa's *Me* → *Where you live* ("The one who looks after your papers can see your town.") → *Choose your town* → *Bedok* → "Do you live in Bedok?" → *Yes, I live there*: "Nura knows your town is Bedok." Mei reads it at `GET /profiles/{id}/area` and is refused setting it ("Only the owner of these papers can choose the town.").
- **Local alerts** (E09-07): dengue in Penang is on the Malaysian deployment. Stop `make dev` and start it again with `NURA_REGION=MY` (a separate database: `NURA_DATABASE_URL=sqlite+aiosqlite:///./dev-my.db make migrate` first, then the same `NURA_DATABASE_URL` on `make dev`). Sign in with a `+60` number, tell Nura about his sugar (onboarding, or `PUT /profiles/{id}/settings` with `{"language": "en", "conditions": ["diabetes"]}` — the PUT is the whole settings screen, so on a fresh profile that is all it needs), set his town to *Air Itam* on *Me*, and move the clock a day (`POST /dev/clock` `{"at": "2026-09-15T10:00:00+08:00"}`): on Today, "More dengue near you this week" — "Start today: empty the water in pots and pails at home.", "Do it at least once a week.", what to do, "This comes from MyHEALTH Kementerian Kesihatan.", the boundary last, and "You are seeing this because it is near your home." A profile with no condition that makes dengue matter (only *joints*, say) never gets it. Back on Singapore, a *breathing* or *heart* condition gives "Haze today: stay indoors more" for the whole region.
- **Food and the seasons**: with *diabetes* told, one food card a week ("This week: half brown rice"), one choice and its source. From Friday 4 September, "Mooncakes: just a small slice" before the Mid-Autumn Festival ("The Mid-Autumn Festival is on Friday 25 September."). Ramadan is never guessed and never added by the family: Pa says yes on *Me* → *Ramadan* ("The one who looks after your papers will see this too."), and Mei's `POST …/search-jobs` for it is refused. With *diabetes* told, "Ramadan: plan it with your doctor" shows from two months before it — see the doctor 1 to 2 months before, what to eat, and "If you feel shaky, sweaty or confused, check your blood sugar at once." (move the clock to `2027-01-04T10:00:00+08:00` to see it; before then there is nothing). With his kidneys on his record there is no food card: a kidney diet is his dietitian's to set.
- **A card made while he reads** (the refresh race): open *See more for you*, page down one card, and — while the fresh page is still on its way — a card made since (a memo card from checkpoint 27's post-visit card) lands as the next card under the one on screen. Nothing he has read moves.

**On your iPhone.** On the same Wi-Fi, `make web` (terminal 2) answers on the Mac's address (`ipconfig getifaddr en0`): open `http://<that address>:5173/app/` in Safari and walk the same steps with the same numbers. The still loads and *Play* reads the card only on a tap; with the phone on silent the captions still show the line being said. "Watch the whole video on …" opens Safari's own tab on the publisher's site, and nothing of it plays inside Nura. The ask bar's filters are in the smaller look (Mei's phone); Pa's phone, in the big and simple look, has his records only.

What you will see (the operator's walk, `make web-e2e` with `make dev` serving the build):

```
✓ formats.spec.ts › a clip: its still from Nura's own server, Play on a tap only, the line being said under it, the whole video on the publisher's site
✓ formats.spec.ts › his week in 30 seconds: his own numbers over the still, no boundary, no link
✓ formats.spec.ts › the chief's Home: Watching for Pa with sources and how often, pause and add; Sent to Pa this week with what became of each card, counting nothing
✓ formats.spec.ts › the ask bar's filters: Records, Web, Providers, Videos — the web and videos from the allowlist only
✓ formats.spec.ts › his town and Ramadan on Me, on his yes; his chief reads his town and cannot set it or add Ramadan; his own ask bar is his records only
✓ day.spec.ts › the post-visit card on the web: each line with where it was said, one left out, his yes, the memo card; the dose change stays a question; the memo card's clips
```

**What "passed" means.** Every card line, why and source is the backend's, in its order; a clip's picture and captions come from Nura's own server and nothing comes from, or goes to, a video site until he taps the link; nothing plays until tapped and the next card never starts itself; the patient keeps his gate (at most two new cards today) and Mei's list has none; every card has *Not for me* and *Hear*; there is no count, badge or reward anywhere; a local alert reaches only a profile whose record makes it matter, in the town he said yes to; and a card made while he reads is never lost. If a step does not do that, tell the operator which one and what you saw instead.
