...
- **The chief's Home** (spec §1): in Mei's window, Today in the smaller look has **Watching for Pa** — "Explainers about blood pressure", "Explainers about amlodipine" once a tablet is on his list, each with its sources and how often — and **Sent to Pa this week**, each card with what became of it ("Pa opened this card.", "Pa heard this card.", "Nura kept this back from Pa."), where it came from, *Hear*, and *Pause this watch* on a card a watch found. *Watch for something new* → *Ramadan*: "Ramadan, before it comes" is on the list, weekly. *Pause* on a watch: it says *Paused*, and *Start again* undoes it. No count of anything anywhere.
...
- **His town** (E09-07): Pa's *Me* → *Where you live* → *Choose your town* → *Bedok* → "Do you live in Bedok?" → *Yes, I live there*: "Nura knows your town is Bedok." Mei reads it at `GET /profiles/{id}/area` and is refused setting it ("Only the owner of these papers can choose the town.").
- **Local alerts** (E09-07): dengue in Penang is on the Malaysian deployment. Stop `make dev` and start it again with `NURA_REGION=MY` (a separate database: `NURA_DATABASE_URL=sqlite+aiosqlite:///./dev-my.db make migrate` first, then the same `NURA_DATABASE_URL` on `make dev`). Sign in with a `+60` number, tell Nura about his sugar (onboarding, or `PUT /profiles/{id}/settings` with `"conditions": ["diabetes"]`), set his town to *Air Itam* on *Me*, and move the clock a day (`POST /dev/clock` `{"at": "2026-09-15T10:00:00+08:00"}`): on Today, "More dengue in Air Itam this week" — "Keep pots and pails at home dry today.", what to do, "This comes from MyHEALTH Kementerian Kesihatan.", the boundary last, and "You are seeing this because you live near there." A profile with no condition that makes dengue matter (only *joints*, say) never gets it. Back on Singapore, a *breathing* or *heart* condition gives "Haze today: stay indoors more" for the whole region.
- **Food and the seasons**: with *diabetes* told, one food card a week ("This week: half brown rice"), one choice and its source. From Friday 4 September, "Mooncakes, the small way" before the Mid-Autumn Festival ("The Mid-Autumn Festival is on Friday 25 September."). Ramadan is never guessed: Mei adds it on *Watching for Pa*, and the card "Ramadan: plan it with your doctor" shows only from six weeks before it (move the clock to `2027-01-04T10:00:00+08:00` to see it; before then there is nothing).
...
✓ formats.spec.ts › his town on Me, on his yes; his chief reads it and cannot set it
✓ day.spec.ts › the post-visit card on the web: each line with where it was said, one left out, his yes, the memo card; the dose change stays a question; the memo card's clips
```
