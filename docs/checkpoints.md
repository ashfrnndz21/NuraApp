...
**The for-someone door (E01-01).** Sign out, sign in with a fresh number, *This is for someone else*: who you are to them is a set of choices (*Their daughter*, *Their son*, …), never typed; the claim says it to him in whole words ("Mei, your daughter, made this for you."). On a phone whose browser has the Contact Picker (Chrome on Android), *Choose from my contacts* fills the number and the name from one contact; Safari on iOS has no picker, so the number is typed there (ADR 0001's table).
