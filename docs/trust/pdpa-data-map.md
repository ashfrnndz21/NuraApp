...
A note on two rows. `profile_id` is an identifier wherever it appears: it is the link from any row to the person it is about. `state_snapshot.family` is an identifier because it names who holds keys; the other five dimensions are health.

<!-- data-map:begin -->
23 tables, 250 columns: 51 identifier, 81 health, 19 consent, 11 audit, 88 operational. Generated by `python3 -m scripts.data_map`; do not edit by hand.

| Table | Column | Type | Nullable | Classification |
|---|---|---|---|---|
| `person` | `id` | Uuid | no | operational |
...
| `profile` | `id` | Uuid | no | operational |
...
| `artifact` | `id` | Uuid | no | operational |
| `artifact` | `kind` | Enum | no | health |
| `artifact` | `storage_key` | String | no | health |
| `artifact` | `content_type` | String | no | operational |
...
| `confirmation` | `id` | Uuid | no | operational |
| `confirmation` | `person_id` | Uuid | no | identifier |
| `confirmation` | `subject` | Enum | no | operational |
| `confirmation` | `subject_id` | Uuid | yes | operational |
...
| `episode` | `id` | Uuid | no | operational |
...
| `appointment` | `id` | Uuid | no | operational |
...
| `consent` | `id` | Uuid | no | operational |
...
| `review_card` | `id` | Uuid | no | operational |
...
| `fact` | `supersedes_id` | Uuid | yes | operational |
...
| `audit_entry` | `id` | Uuid | no | operational |
...
| `medication_line` | `id` | Uuid | no | operational |
...
| `medication_line` | `source_kind` | Enum | no | operational |
...
| `medication_line` | `supersedes_id` | Uuid | yes | operational |
...
| `review_field` | `id` | Uuid | no | operational |
...
| `review_field` | `state` | Enum | no | operational |
...
| `state_snapshot` | `id` | Uuid | no | operational |
...
| `state_snapshot` | `trigger` | Enum | no | operational |
| `state_snapshot` | `trigger_fact_id` | Uuid | yes | health |
| `state_snapshot` | `supersedes_id` | Uuid | yes | operational |
...
| `stewardship` | `id` | Uuid | no | operational |
...
| `dose_taken` | `id` | Uuid | no | operational |
...
| `interaction_flag` | `id` | Uuid | no | operational |
...
| `medication_supply` | `id` | Uuid | no | operational |
...
| Speech and model providers (transcription, the sentence the model writes) | Recording audio; the words of a memo | Region-pinned endpoints; no training on inputs; the port is `app/llm/` and `app/channels/whatsapp/provider.py` when they land |
...
| Named | *to be named by the owner before checkpoint 11* |
