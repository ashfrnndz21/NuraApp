...
- **On its way in** (#129, ADR 0013): while the visit is recorded, the phone keeps the audio in its memory until the doctor's yes; nothing of it reaches the server before (a chunk sent before it is refused). After his yes the phone sends the audio in chunks, from the notice on, so his answer is in the first of them. The chunks wait in the region's object store under `consult-uploads/<profile>/<upload>/`; no route reads them back, and they are not an artefact, a transcript or a card. They become the recording on Stop. They are deleted when the page is left before Stop, when the visit is not finished within two and a half hours, when the RECORDING consent is withdrawn, and when the account is closed. Before #129 nothing left the phone until Stop (ADR 0006).
...
- **The demo-only processor**: outside a recording's own path, a page's bytes reach a third party today only through the Claude-backed runtime features (E02's document extractor; the feed's searcher and compressor when they land), and only on a declared demo, never a real deployment (ADR 0017). That processor is Anthropic. It is named here because it is the same kind of fact as the row above it — bytes leaving the region to a named third party — even though it is not the recording pattern's own path.
