...
The first line says what Nura did on that surface, and no more (`WHAT_NURA_DID`). A surface that renders without its line is a defect. `tests/test_boundary.py:95` holds the register to the table below; `tests/test_boundary.py:105` shows the one surface on main today carrying it.
...
|---|---|---|---|---|---|
| `state_posture` | The posture of the day — stable, watch, act — and the six dimensions, from facts already confirmed. Posture is the wash on the screen and the order of the feed, never a finding. | The person's own record only: a posture rises from a clinician's control word already in the record, a discharge, an admission, a visit window. No threshold on any reading lives in State (`app/state/dimensions.py:1`). | No | Nura sorts and prepares. | State never rises from a number: `tests/test_state.py:130`; it follows the clinician's word: `tests/test_state.py:149`; an unreadable word is kept and not settled: `tests/test_state.py:194`; a disputed fact does not move State: `tests/test_state_acceptance.py:142`; the line is on `GET /state`: `tests/test_boundary.py:105` |
...
| `brief` | The pre-visit brief: what changed since the last visit, from the record. (E05, not on main yet.) | The person's own record; no clinical rule. | No | Nura prepared this from your papers. | The line exists in every language: `tests/test_boundary.py:38`; the surface's own test is owed by the E05 PR (follow-up note in the E16 PR) |
| `questions` | Questions for the doctor generated from the record: gaps, changes, things the person said. (E05-02.) | The person's own record; any medical framing is a question, not a statement. | No | Nura wrote these questions for you to ask Dr Tan. | `tests/test_boundary.py:38`, `:73` (no line starts, stops or changes a medicine); the surface's own test owed by E05 |
| `summary` | What the doctor said, from the recording, in the patient's language, and the memo in his words. (E05.) | The doctor's own words; Nura restates, it does not add. | No | Nura wrote down what Dr Tan said. | `tests/test_boundary.py:38`; runtime text passes `plain_words.verify` before it reaches him (`tests/test_plain_words.py`); the surface's own test owed by E05 |
| `learning_card` | Which short explanation to show, chosen from State. (E21.) | Content written or licensed in advance; nothing is generated per person. | No | This explains one thing in simple words. | `tests/test_boundary.py:38`; every card names its State (`tests/test_state.py`); the surface's own test owed by E21 |
| `feeling_inference` | A pattern in how he said he feels, offered as a thing to talk about. (E17-02.) | The person's own check-ins; no diagnostic rule. | No | Nura only noticed a pattern to talk about. | `tests/test_boundary.py:38`, `:73`; the surface's own test owed by its PR |
| `not_feeling_well` | Who to tell and whether today or not a worry, from the roster and the person's own discharge instructions. | The person's own hospital letter and conditions (the red-flag list in `.claude/rules/safety.md` is the letter's own words, surfaced as "Dr Tan wrote this in your hospital letter"). | No | Nura does not decide what is wrong. | `tests/test_boundary.py:38`, `:73`; the surface's own test owed by its PR |
...
6. **The learning supply.** An explainer about a condition, shown to a person with that condition, is not advice, but it is close. Mitigation: content is chosen from State, not generated; every card names its State; nothing is personalised beyond selection.
