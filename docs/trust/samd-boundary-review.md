...
| `learning_card` | Which short explanation to show, chosen from State. (E21.) | Content written or licensed in advance; nothing is generated per person. | No | Nura explains one thing in simple words. | `tests/test_boundary.py`; every card names its State and, for an inferring surface, its line (`tests/test_state.py`); the surface's own test owed by E21 |
