...
| `brief` | The pre-visit brief: what changed since the last visit, from the record, and every symptom written down since then on a line of its own in the symptom log's words (E14-01); rendered three days before the visit by the delivery engine (E05-01, rule `brief_three_days_before`), on one page (at most 21 lines, the rest folded into a count). Its lines come from the record and are not edited; what the family adds goes on the questions card (E05-02). | The person's own record; no clinical rule. | No | Nura prepared this from your papers. | `tests/test_boundary.py` (the line in every language; a brief row cannot be written without it); `tests/test_brief_b1.py` (symptoms as their own lines, never under his papers; one page; T-3, once, under the cap) |
...
| `not_feeling_well` | Which of four rows to show, top row wins: a red flag (the ambulance now), call the doctor's clinic today (a symptom that is not a red flag said "quite a lot" or more, or lasting a day or more, or on the licensed monograph of a medicine started in the last fourteen days), a tablet with no Taken (ask the doctor before taking it), rest; and who to tell. On WhatsApp a red flag's step is by tier and by the doctor's hours: chest pain, breathless at rest and the signs of a stroke are the ambulance at any hour; the rest are the doctor today, and out of the doctor's hours the emergency department of the hospital marked as on his insurance, or the emergency number if it gets worse (E19-05). | The person's own hospital letter and conditions (the red-flag list in `.claude/rules/safety.md`, one table in `app/safety/red_flags.py`, with its ambulance tier as a subset of it); a medicine's watch-outs from the licensed monograph by rule id, the rule the feeling cloud reads (E17). | No | You did right to say so. / Nura wrote down how you feel. Every card, whatever its row, ends "Nura does not decide what is wrong."; on a red flag the card is urgent: the reassurance, the calls, then that line alone — never "Ask your doctor." after an emergency number. | `tests/test_boundary.py` (words in every language; no line starts, stops or changes a medicine; the reassurance first; the letter's words only in the letter's name; the last line on every card); `tests/test_call_clinic.py` (every combination of the table and the row that wins; no card names an amount); `tests/test_red_flag_escalation.py` (tier × the doctor's hours × a hospital marked; within one minute) |
| `trend` | The lab trend (E09-01): his confirmed results for one analyte, oldest first, each placed against the reference range that fits him on the day — his age band read from the decade he was born in, his sex when the record holds it, and the lab's own printed range when the paper names the lab — and the direction over the last three results (up, down, about the same, up and down), by arithmetic only. | Published guideline tables and the lab's own printed ranges, each row named by source id in `backend/tests/fixtures/labs/ranges.json` (NCEP ATP III 2001, ADA Standards of Care 2024 §2, KDIGO 2012, AACB harmonised intervals 2014, WHO 2011, Surks & Hollowell 2007); a fixture until a clinical reviewer signs off a licensed table. Where his number sits (inside, above, below) is said only against the range the lab printed on his paper — the lab's own flag, never Nura's threshold; against a guideline range the range is shown and nothing is said about where he sits. No cause, no medicine, no instruction. | No | Nura put your blood tests side by side. | Every line in every language passes plain words, a word that places the number is only ever beside the lab's range, no line names a cause or a medicine, and the trend ends on the line: `tests/test_trends.py`; a trend cannot be rendered from a stale State and its row carries the line: `tests/test_trends.py`; over HTTP in Malay: `tests/test_trends_api.py` |
