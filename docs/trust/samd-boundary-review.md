...
| `feeling_inference` | A pattern in how he said he feels, offered as a thing to talk about. (E17-02.) | The person's own check-ins, read against a new medicine's licensed monograph rule ids, the direction of his own blood pressure, and a visit or discharge this week; no diagnostic rule, no threshold. | No | Nura noticed this in how you said you feel. | `tests/test_boundary.py` (words in every language; no line starts, stops or changes a medicine in any of them; the reassurance first; the letter's words only in the letter's name); the surface's own: `backend/tests/test_feelings_inference.py` (every note ends on this line and is rendered from State; at most two things to tell the doctor, each naming its reason by id; a medicine only where its licensed monograph lists the feeling; no template starts, stops or changes a medicine or names a condition, in any language — with one carve-out: right after a line that names a medicine, two fixed lines (`DO_NOT_STOP`, #157) say not to stop it himself and to tell the doctor how he feels, in English, Malay and Chinese ("Do not stop {medicine} yourself." / "Jangan berhenti makan {medicine} sendiri." / "不要自己停{medicine}。"). This keeps the medicine as it is rather than changing it — the decision stays the doctor's — and is held to that exact shape by a whitelist, not a blacklist (`app/safety/plain_words.py:_keep_taking_pattern`). Unsigned wording: on the clinician sign-off list, not yet approved (`docs/trust/clinical-wording-sign-off.md`); a red word or a yes that makes one red takes the red-flag path and makes no note) |
