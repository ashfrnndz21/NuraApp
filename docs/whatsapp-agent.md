...
- A red-flag word from someone on more than one family's list, with nothing to say which, is raised on every profile where their key holds the emergency card, each family told it may be about theirs; the sender is asked which, and the name they send back stops the other ladders. The message itself is kept on none of them.
