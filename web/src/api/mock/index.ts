...
function gapsFor(bio: BiographyOut | undefined, deferred: Record<string, number>, someoneIn: boolean): PlanCardOut[] {
  ...
  // A tap gap names the word whose follow-up question fills it; the invite gap closes once
  // anyone holds a key (asked of the live API).
  const WORD: Record<string, string> = { all: "drug_all", thin: "thinner" };
...
export function mockTransport(path: string, call: Call, passthrough: Passthrough): Promise<unknown> | undefined {
  ...
  if (resource === "plan") {
    ...
    const bio = store.biography.get(profileId);
    return passthrough<KeyOut[]>(`/profiles/${profileId}/keys`, { token: call.token ?? null })
      .catch(() => [] as KeyOut[])
      .then((keys) => {
        const someoneIn = Array.isArray(keys) && keys.some((key) => key.revoked_at === null);
        const plan: PlanOut = { profile_id: profileId, state_id: STATE_ID, cards: gapsFor(bio, deferred, someoneIn) };
        return clone(plan);
      });
  }
