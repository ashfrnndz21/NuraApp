 ...
 *  tab bar, until the sitting itself is done — with one way on, "Next", which runs what
 *  "Looks right" used to run at once (`continueAfterPaper`) and returns to the step that would
 *  have come next: never a dead end, and never a step skipped or reordered. A failure on the
 *  way out (the sitting's own read, offline) is said here, with the same "Next" to try again —
 *  never a silent stall on a screen with nothing left to tap. */
export function InsightStep({ card }: { card: ReviewCardOut }): JSX.Element {
  ...
  return (
    <main class="screen onboarding" data-stage="insight" data-card-id={card.card_id}>
      ...
      <PaperInsightView card={card} leaveLabel={s.onboarding.next} onLeave={() => void next()} testId="insight-body" />
      <Notice error={error} />
      {error && (
